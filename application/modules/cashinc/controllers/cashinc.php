...
class Cashinc extends MX_Controller
{
    ...
    function confirmation($pid)
    {
        ...
        else
        {
            ...
            $this->ledger->add($this->get_acc_type($cash1->acc), "CR-000".$cash1->no, $cash1->currency, $cash1->dates, $cash1->amount, 0);
             ...
             $this->session->set_flashdata('message', "$this->title CR-000$cash->no confirmed..!");
        }
        redirect($this->title);
    }
...
//    ===================== approval ===========================================
	...
	// Form validation
        ...
        {
            $this->model->customer = $this->customer->get_customer_id($this->input->post('tcustomer'));
...
//            echo validation_errors();
	...
	$data['form_action'] = site_url($this->title.'/update_process/'.$cash->id);
        ...
        $data['default']['customer'] = $this->customer->get_customer_name($cash->customer);
...
// ===================================== PRINT ===========================================
   ...
   function invoice($po=null)
   {
       ...
       $data['customer'] = $this->customer->get_customer_name($cash->customer);
       $data['desc'] = $cash->desc;
...
// ====================================== REPORT =========================================
    ...
    function report_process()
    {
        ...
        $cust = $this->input->post('tcustomer');
...
//        Property Details
    ...
    {
       if ($cust) { $this->model->where('customer', $this->customer->get_customer_id($cust)); }
