...
class Ar_refund_backup extends MX_Controller
{
    function __construct()
    {
        parent::__construct();
        
        $this->load->model('Ar_refund_backup_model', '', TRUE);
    ...
    private $customer,$user,$journal,$cek,$sales,$over;
	...
	// ---------------------------------------- //
        $appayments = $this->Ar_refund_backup_model->get_last($this->modul['limit'], $offset)->result();
        $num_rows = $this->Ar_refund_backup_model->count_all_num_rows();
	...
	$data['form_action'] = site_url($this->title.'/search');
        ...
        $appayments = $this->Ar_refund_backup_model->search($this->input->post('tno'), $this->input->post('tcust'), $this->input->post('tdate'))->result();
...
//        Property Details
        ...
        $data['reports'] = $this->Ar_refund_backup_model->report($customer,$start,$end,$acc,$cur)->result();

        $total = $this->Ar_refund_backup_model->total($customer,$start,$end,$acc,$cur);
