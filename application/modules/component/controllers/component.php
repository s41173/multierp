...
//
        ...
        $this->table->set_heading('#','No', 'Name', 'Pbl', 'Status', 'Act', 'Limit', 'Role', 'Closing-Check', 'Action');
        ...
        foreach ($components as $component)
        {
            ...
            $this->table->add_row
            (
                form_checkbox($datax), ++$i, $component->name, $component->publish, $component->status, $component->aktif, $component->limit, $component->role, $this->stts($component->closing),
...
//                anchor_popup($this->title.'/update/'.$component->id, '<span>update</span>', $atts).' '.
    ...
    function delete($uid)
    {
        ...
        $img = $img->icon;
        if ($img != 'default.png'){ $img = "./images/component/".$img; unlink("$img"); }
	...
	$data['link'] = array('link_back' => anchor($this->title,'<span>kembali</span>', array('class' => 'back')));
        ...
        $this->form_validation->set_rules('ttable', 'Table Name', 'required');
        $this->form_validation->set_rules('rtruncate', 'Truncate', 'required');
        $this->form_validation->set_rules('rclose', 'Closing Check', 'required');
        ...
        if ($this->form_validation->run($this) == TRUE)
        {
            ...
            if ( !$this->upload->do_upload("userfile")) // if upload failure
            {
                ...
                $component = array('name' => $this->input->post('tname'), 'title' => $this->input->post('ttitle'),
                                   ...
                                   'icon' => 'default.png', 'table' => $this->input->post('ttable'), 'truncate' => $this->input->post('rtruncate'),
                                   'closing' => $this->input->post('rclose'));
            }
            else
            {
                ...
                $component = array('name' => $this->input->post('tname'), 'title' => $this->input->post('ttitle'),
                                   ...
                                   'aktif' => $this->input->post('raktif'), 'limit' => $this->input->post('tlimit'),
                                   'role' => $this->split_array($this->input->post('crole')), 'order' => $this->input->post('torder'),
                                   'truncate' => $this->input->post('rtruncate'), 'closing' => $this->input->post('rclose'),
                                   'icon' => $info['file_name']);
            }
	...
	$data['default']['name'] = $component->name;
        ...
        $data['default']['table'] = $component->table;
        $data['default']['truncate'] = $component->truncate;
        $data['default']['close'] = $component->closing;
	...
	$data['link'] = array('link_back' => anchor($this->title,'<span>kembali</span>', array('class' => 'back')));
        ...
        if ($this->form_validation->run($this) == TRUE)
        {
            ...
            {
                $data['error'] = $this->upload->display_errors();
                $component = array('name' => $this->input->post('tname'), 'title' => $this->input->post('ttitle'), 'closing' => $this->input->post('rclose'),
                                   'publish' => $this->input->post('rpublish'), 'status' => $this->input->post('cstatus'), 'truncate' => $this->input->post('rtruncate'),
                                   'aktif' => $this->input->post('raktif'), 'limit' => $this->input->post('tlimit'), 'table' => $this->input->post('ttable'),
                                   'role' => $this->split_array($this->input->post('crole')), 'order' => $this->input->post('torder'));
            }
            else
            {
