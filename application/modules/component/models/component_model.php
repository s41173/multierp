...
class Component_model extends CI_Model
{
    ...
    {
        $this->db->select('id, name, title, publish, status, aktif, limit, role, icon, order, closing');
    ...
    {
        $this->db->select('id, name, title, publish, status, aktif, limit, role, icon, order, table, truncate, closing');
        $this->db->where('id', $uid);
