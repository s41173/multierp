...
class Nar_refund extends MX_Controller
{
    function __construct()
    {
        parent::__construct();
        
        $this->load->model('Nar_refund_model', '', TRUE);
        $this->load->model('Nar_refund_trans_model', '', TRUE);
    ...
    private $customer,$user,$journal,$cek,$sales,$over,$currency,$unit;
	...
	// ---------------------------------------- //
        $appayments = $this->Nar_refund_model->get_last($this->modul['limit'], $offset)->result();
        $num_rows = $this->Nar_refund_model->count_all_num_rows();
	    ...
	    $config['base_url'] = site_url($this->title.'/get_last');
            ...
            foreach ($appayments as $appayment)
            {
                ...
                $this->table->add_row
                (
                    ++$i, 'NCF-00'.$appayment->no, tgleng($appayment->dates), $appayment->prefix.' '.$appayment->name, $this->acc($appayment->acc).' - '.$appayment->currency, $appayment->check_no, number_format($appayment->amount),
	...
	$data['form_action'] = site_url($this->title.'/search');
        ...
        $appayments = $this->Nar_refund_model->search($this->input->post('tno'), $this->input->post('tcust'), $this->input->post('tdate'))->result();
...
//    ===================== approval ===========================================
    ...
    {
        $appayment = $this->Nar_refund_model->get_nar_refund_by_id($pid)->row();
...
//
            if ($total == 0)
            {
              $this->session->set_flashdata('message', "NRF-00$appayment->no has no value..!");
              redirect($this->title);
            }
            elseif ($this->valid_check_no($appayment->no,$pid) == FALSE )
            {
                $this->session->set_flashdata('message', "NRF-00$appayment->no check no registered..!");
                redirect($this->title);
            }
            else
            {
                ...
                $this->Nar_refund_model->update_id($pid, $data);
                ...
                $this->journal->create_journal($appayment->dates, $appayment->currency,
                                              ...
                                              'NRF', $appayment->no, 'AP', $appayment->amount);

               $this->session->set_flashdata('message', "$this->title NRF-00$appayment->no confirmed..!"); // set flash data message dengan session
               redirect($this->title);
            }
        }
    ...
    {
        $val = $this->Nar_refund_trans_model->get_last_item($no)->result();
    ...
    private function undo_confirmation($no,$cust,$cur)
    {
        ...
        { $this->over->add_undo($value->no, $cust, $value->nsales_no, $value->nar_payment, $value->balance, $value->over, $cur); }
    }

    private function get_trans_code($po)
    {
        $val = $this->Nar_refund_trans_model->get_last_item($po)->result();
        $ress=null;

        foreach ($val as $res)
        { $ress = $ress.'NSOV-00'.$res->no.','; }
    ...
    {
        $val = $this->Nar_refund_model->get_nar_refund_by_no($no)->row();
        if ($val->check_no != null)
        {
            if ($this->Nar_refund_model->cek_no($val->check_no,$pid) == FALSE)
    ...
    {
        $appayment = $this->Nar_refund_model->get_nar_refund_by_no($po)->row();

        if ( $appayment->approved == 1 )
        {
           $this->session->set_flashdata('message', "Can't change value - NRF-00$po approved..!"); // set flash data message dengan session
...
//    ===================== approval ===========================================
    ...
    function delete($uid,$po)
    {
        ...
        $appayment = $this->Nar_refund_model->get_nar_refund_by_id($uid)->row();

        if ($appayment->approved != 1)
        {
            $this->Nar_refund_trans_model->delete_payment($po); // model to delete appayment item
            $this->Nar_refund_model->delete($uid); // memanggil model untuk mendelete data
            $this->session->set_flashdata('message', "1 $this->title successfully removed..!");
        }
        else
        {
            if ( $this->journal->cek_approval('NRF',$po) == TRUE ) // cek journal harian sudah di approve atau belum
            {
                $this->undo_confirmation($po,$appayment->customer,$appayment->currency);
                $this->journal->remove_journal('NRF',$po); // delete journal
	...
	$data['form_action'] = site_url($this->title.'/add_process');
	$data['currency'] = $this->currency->combo();
        $data['code'] = $this->Nar_refund_model->counter_no();
        $data['user'] = $this->session->userdata("username");
...
//        $this->form_validation->set_rules('tcheck', 'Check No', 'callback_valid_check|callback_valid_check_no');
        ...
        if ($this->form_validation->run($this) == TRUE)
        {
            $appayment = array('customer' => $this->customer->get_customer_id($this->input->post('tcustomer')),
                               'no' => $this->Nar_refund_model->counter_no(), 'notes' => $this->input->post('tnotes'),
...
//
            $this->Nar_refund_model->add($appayment);
...
//        ============================ Item  =========================================
        $items = $this->Nar_refund_trans_model->get_last_item($po)->result();
...
//        $this->db->select('id, ap_payment, code, no, notes, amount');
        ...
        foreach ($items as $item)
        {
            $this->table->add_row
            (
                ++$i, 'NSOV-00'.$item->no, 'NSO-00'.$item->nsales_no.' / NCR-00'.$item->nar_payment, number_format($item->over),
...
//    ======================  Item Transaction   ===============================================================
    ...
    function add_item($po=null)
    {
        ...
        if ($this->form_validation->run($this) == TRUE)
        {
            ...
            $pitem = array('nar_refund' => $po, 'no' => $over->no, 'nsales_no' => $over->nsales_no,
                           'nar_payment' => $over->nar_payment, 'balance' => $over->balance, 'over' => $over->over);

            $this->Nar_refund_trans_model->add($pitem);
    ...
    {
        $totals = $this->Nar_refund_trans_model->total($po);
        $appayment = array('amount' => $totals['over']);
	$this->Nar_refund_model->update($po, $appayment);
    }
    ...
    function delete_item($id,$po)
    {
        ...
        $this->Nar_refund_trans_model->delete($id); // memanggil model untuk mendelete data
        $this->update_trans($po);
...
//    ==========================================================================================
    ...
    {
        if ($this->Nar_refund_trans_model->valid_trans($so,$ar_refund) == FALSE)
        {
            $this->form_validation->set_message('valid_refund_trans', "NSOV already registered..!");
            return FALSE;
        }
        else { return TRUE; }
    }
...
//
       $data['items'] = $this->Nar_refund_trans_model->get_last_item($po)->result();
...
//        Property Details
        ...
        $data['reports'] = $this->Nar_refund_model->report($customer,$start,$end,$acc,$cur)->result();

        $total = $this->Nar_refund_model->total($customer,$start,$end,$acc,$cur);
