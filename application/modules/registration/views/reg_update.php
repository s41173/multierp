...
function calculate_cost()
{
	...
	var res = p3-p1;
	if (res > 0){ document.getElementById("cptype").value = "credit"; }else {document.getElementById("cptype").value = "settled";  }
}
...
<div class="rightcol" style="width:49%; float:right;">
 ...
 <fieldset class="field"> <legend> Registration Form </legend>
		<table>
            ...
            <td> <?php $js = 'id="cfacultys"'; echo form_dropdown('cfaculty', $faculty, isset($default['faculty']) ? $default['faculty'] : '', $js); ?>
    ...
    <fieldset class="field"> <legend> Cost Details </legend>
		...
		<table>
          ...
          <tr> <td> <label for="tp2date"> II - Paid Date </label> </td> <td>:</td> 
          <td> <input type="Text" name="tp2date" id="d3" title="II Payment" size="10" readonly class="required"
               value="<?php echo set_value('tp2date', isset($default['p2date']) ? $default['p2date'] : ''); ?>" /> 
               <img src="<?php echo base_url();?>/jdtp-images/cal.gif" onClick="javascript:NewCssCal('d3','yyyymmdd')" style="cursor:pointer"/> 
           </td> 
          </tr>
          
          <tr> <td> <label for="tstatus"> Status </label> </td> <td>:</td> 
          <td> 
          <input type="text" name="cptype" size="10" readonly id="cptype" value="<?php echo set_value('cptype', isset($default['ptype']) ? $default['ptype'] : ''); ?>">
           </td> 
          </tr>
