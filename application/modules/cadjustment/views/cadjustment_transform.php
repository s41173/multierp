...
<script type="text/javascript">
	...
	$('#bget').click(function() {
		...
		$.ajax({
		type: 'POST',
		url: site +"/cadjustment/get_contract_balance",
		data: "co="+ co,
		success: function(data)
		{
		  document.getElementById("tcobalance").value = data;
		}
		})
		return false;
	});
...
<div id="webadmin">
	...
	<fieldset class="field"> <legend> Contract Adjustment </legend>
	<form name="modul_form" class="myform" id="ajaxform" method="post" action="<?php echo $form_action; ?>" >
				<table>
                
					<tr>	
						<td> <label for="tno"> No - COA-00 </label> </td> <td>:</td>
	     <td> <input type="text" class="required" name="tno" id="tno" size="4" readonly title="Name" onkeyup="checkdigit(this.value, 'tno')"
		       value="<?php echo isset($code) ? $code : ''; ?>" /> &nbsp; <br /> </td>
					...
					<tr>
						...
						<td> <input type="text" class="" name="tdocno" size="15" title="Document No"
                             value="<?php echo set_value('tdocno', isset($default['docno']) ? $default['docno'] : ''); ?>" /> &nbsp; <br />
                        </td>
					</tr>
                    
                    <tr> 
						<td> <label for="tcustomer"> Contract - CO-00 </label> </td> <td>:</td>
						<td> <input type="text" readonly="readonly" class="required" name="tcontract" id="tcust" size="4" title="Contract"
                             value="<?php echo set_value('tcontract', isset($default['contract']) ? $default['contract'] : ''); ?>" /> &nbsp; 
						<?php echo anchor_popup(site_url("contract/get_list/null/"), '[ ... ]', $atts1); ?> - 
                        <input type="text" readonly="readonly" name="tcobalance" id="tcobalance" size="10" />
                        <input type="button" id="bget" value="GET AMOUNT" />
                        </td>
					</tr>
                    
                    <tr>
						<td> <label for="ttotal"> Total </label> </td>  <td>:</td>
						<td> <input type="text" class="required" id="ttotal" name="ttotal" size="15" title="Total" onKeyUp="checkdigit(this.value,'ttotal')"
                             value="<?php echo set_value('ttotal', isset($default['total']) ? $default['total'] : ''); ?>" /> &nbsp; <br /> </td>
					</tr>
					
					<tr>
						<td> <label for="tnote"> Note </label> </td>  <td>:</td>
						<td> <input type="text" class="required" id="tnote" name="tnote" size="50" title="Note"
                             value="<?php echo set_value('tnote', isset($default['note']) ? $default['note'] : ''); ?>" /> &nbsp; <br /> 
                        </td>
					</tr>
