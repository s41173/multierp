...
class Generations extends MX_Controller
{
    ...
    function get_last()
    {
        ...
        $data['main_view'] = 'generation_view';
...
//
            ...
            $this->table->set_heading('No', 'Generation', 'Description', '#');
//
            $i = 0 + $offset;
...
//
	...
	// Form validation
        $this->form_validation->set_rules('tgeneration', 'Generation', 'required|numeric|callback_valid_generation');
        $this->form_validation->set_rules('tdesc', 'Description', '');
        
        if ($this->form_validation->run($this) == TRUE)
        {
            
            $this->model->name = $this->input->post('tgeneration');
...
//               $this->load->view('student_form', $data);
    ...
    function update($uid)
    {
        ...
        $data['title'] = $this->properti['name'].' | Administrator  '.ucwords($this->modul['title']);
        $data['h2title'] = $this->modul['title'];
        $data['main_view'] = 'generation_form';
	$data['form_action'] = site_url($this->title.'/update_process');
	$data['link'] = array('link_back' => anchor($this->title,'<span>back</span>', array('class' => 'back')));
        
        $this->model->where('id', $uid)->get();
        
        $data['default']['generation'] = $this->model->name;
        $data['default']['desc']       = $this->model->desc;

	$this->session->set_userdata('curid', $uid);
        $this->load->view('generation_form', $data);
    }
	...
	// Form validation
        $this->form_validation->set_rules('tgeneration', 'Generation', 'required');
        $this->form_validation->set_rules('tdesc', 'Description', '');
...
//            echo 'true'; 
        ...
        else { $this->load->view('generation_form', $data); }
        
       
    }

    public function valid_generation($name)
    {
        $val = $this->model->where('name', $name)->count();

        if ($val > 0)
        {
            $this->form_validation->set_message('valid_generation', "Generation [$name] - Already Registered..!");
            return FALSE;
        }
        else{ return TRUE; }
    }
    ...
    public function report()
    {
        ...
        $this->load->view('generation_report', $data);
    }
