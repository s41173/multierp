...
class Payment extends MX_Controller
{
	...
	// ---------------------------------------- //
        $payments = $this->model->get($this->modul['limit'],$offset);
        $num_rows = $this->model->count();

        if ($num_rows > 0)
        {
	    $config['base_url'] = site_url($this->title.'/get_last_payment');
            ...
            {
                $datax = array('name'=> 'cek[]','id'=> 'cek'.$i,'value'=> $payment->id,'checked'=> FALSE, 'style'=> 'margin:0px');
                
                $this->table->add_row
                (
                    form_checkbox($datax), ++$i, $payment->name,
                    anchor($this->title.'/update/'.$payment->id,'<span>details</span>',array('class' => 'update', 'title' => '')).' '.
                    anchor($this->title.'/delete/'.$payment->id,'<span>delete</span>',array('class'=> 'delete', 'title' => 'delete' ,'onclick'=>"return confirm('Are you sure you will delete this data?')"))
                );
            }
...
//               $this->load->view('template', $data);
	...
	$data['link'] = array('link_back' => anchor('payment/','<span>back</span>', array('class' => 'back')));
        ...
        $payment = $this->model->where('id', $uid)->get();

        $data['default']['name'] = $payment->name;

	$this->session->set_userdata('curid', $payment->id);
    ...
    public function valid_payment($name)
    {
        ...
        if ($val > 0)
        {
            $this->form_validation->set_message('valid_payment', "This $this->title is already registered.!");
            return FALSE;
        }
        else
        {
	...
	if ($val > 0)
        {
            $this->form_validation->set_message('validation_payment', 'This payment is already registered!');
            return FALSE;
        }
        else
        {
