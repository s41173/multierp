...
//        $this->account  = $this->load->library('account_lib');
        ...
        $this->model = new Cost();
        $this->load->model('Cost_model','cm',TRUE);
	...
	// ---------------------------------------- //
	    ...
	    $config['base_url'] = site_url($this->title.'/get_last');
            ...
            foreach ($costs as $cost)
            {
                $this->table->add_row
                (
                    ++$i, $cost->category, $cost->name, $this->account->get_code($cost->account_id).' - '.$this->account->get_name($cost->account_id),
	...
	// Form validation
        ...
        $this->form_validation->set_rules('titem', 'Code', 'required');
...
//            echo validation_errors();
    ...
    public function valid_cost($name)
    {
        ...
        if ($val > 0)
        {
            $this->form_validation->set_message('valid_cost', "Invalid Cost..!");
            return FALSE;
        }
        else{ return TRUE; }
    }
