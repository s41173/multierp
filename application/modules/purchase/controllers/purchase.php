...
//        $this->journal = new Journal_lib();
    ...
    private  $atts = array('width'=> '400','height'=> '350',
                      ...
                      'screeny'=> '0','class'=> 'print','title'=> 'print', 'screeny' => '\'+((parseInt(screen.height) - 350)/2)+\'');
	...
	// ---------------------------------------- //
	    ...
	    $config['base_url'] = site_url($this->title.'/get_last_purchase');
            ...
            foreach ($purchases as $purchase)
            {
                ...
                $this->table->add_row
                (
                    ++$i, 'PO-00'.$purchase->no, $purchase->currency, tglin($purchase->dates), ucfirst($purchase->acc), $purchase->prefix.' '.$purchase->name, $purchase->notes, number_format($purchase->total + $purchase->costs), number_format($purchase->p2), $this->status($purchase->status),
    ...
    function confirmation($pid)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                $this->update_buying($purchase->no);
                ...
                //  create journal
                $this->create_po_journal($pid, $purchase->dates, $purchase->currency, 'PO-00'.$purchase->no.'-'.$purchase->notes, 'PJ',
                                         $purchase->no, 'AP', $purchase->total + $purchase->costs, $purchase->p1,$purchase->p2);

               $this->session->set_flashdata('message', "$this->title PO-00$purchase->no confirmed..!"); // set flash data message dengan session
            }
        }
...
//    ===================== approval ===========================================
    ...
    function delete($uid,$po)
    {
        ...
        if ( $this->valid_period($val->dates) == TRUE && $val->stock_in_stts == 0 && $this->ap->cek_relation_trans($po,'no','PO') == TRUE )
        {
...
//        ============================ Purchase Item  =========================================
        ...
        //Set heading untuk table
        $this->table->set_heading('No', 'Item', 'Qty', 'Unit', 'Unit price', 'Tax', 'Amount', 'Action');
        ...
        foreach ($items as $item)
        {
            $this->table->add_row
            (
                ++$i, 'PRO-0'.$item->product.' : '.$this->product->get_name($item->product), $item->qty, $this->product->get_unit($item->product), number_format($item->price), number_format($item->tax), number_format($item->amount),
...
//            $this->load->view('purchase_transform', $data);
    ...
    public function valid_period($date=null)
    {
        ...
        if (intval($month) != intval($p->month) || intval($year) != intval($p->year))
        {
