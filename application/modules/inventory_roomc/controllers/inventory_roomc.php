...
class Inventory_roomc extends MX_Controller
{
	...
	// ---------------------------------------- //
	    ...
	    $config['base_url'] = site_url($this->title.'/get_last');
            ...
            $this->table->set_heading('No', 'Room', 'Desc', 'Action');
	...
	// Form validation
        $this->form_validation->set_rules('tname', 'Name', 'required|callback_valid_room');
...
//            echo validation_errors();
    ...
    public function valid_room($name)
    {
        $val = $this->model->where('name', $name)->count();

        if ($val > 0)
        {
            $this->form_validation->set_message('valid_room', "Invalid Room..!");
            return FALSE;
        }
        else{ return TRUE; }
    }

    public function validation_room($name)
    {
