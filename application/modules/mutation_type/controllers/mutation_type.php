...
class Mutation_type extends MX_Controller
{
	...
	$this->load->view('mutation_view', $data);
    }
	...
	// Form validation
        $this->form_validation->set_rules('tcode', 'Code', 'required|callback_valid_type');
...
//            echo validation_errors();
    ...
    public function valid_type($code)
    {
        $val = $this->model->where('code', $code)->count();

        if ($val > 0)
        {
            $this->form_validation->set_message('valid_type', "Invalid Mutation Type..!");
            return FALSE;
        }
        else{ return TRUE; }
    }
    ...
    public function validating_type($name)
    {
        ...
        if ($val > 0)
        {
            $this->form_validation->set_message('validating_type', "Invalid Mutation Type..!");
            return FALSE;
        }
        else{ return TRUE; }
    }
