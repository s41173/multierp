...
//    fungsi di panggil ketika ada po yg masih blm approved ketika hendak closing harian
    function cek_approval_po($date,$currency)
    {
        ...
        $query = $this->ci->db->get('vinyl_purchase')->num_rows();
    ...
    function get_po($no)
    {
        ...
        $query = $this->ci->db->get('vinyl_purchase')->row();
        return $query;
    }
    ...
    function get_buying_price($pname,$no)
    {
        ...
        $this->ci->db->where('vinyl_purchase_id', $no);
        $this->ci->db->where('product', $pname);
        $query = $this->ci->db->get('vinyl_purchase_item')->row();
    ...
    function settled_po($uid, $users)
    {
        ...
        $this->ci->db->update('vinyl_purchase', $users);
    }
...
//    ======================= relation cek  =====================================
    ...
    function closing()
    {
        ...
        $query = $this->ci->db->get('vinyl_purchase')->result();
    ...
    private function delete($po)
    {
       ...
       $this->ci->db->delete('vinyl_purchase_item');

       $this->ci->db->where('no', $po);
       $this->ci->db->delete('vinyl_purchase');
    }
