...
class Stock_adjustment_temp_lib {
    ...
    {
        $this->ci->db->where('product_id', $pid);
        $this->ci->db->where('stock_adjustment', $so);
        $res = $this->ci->db->get('stock_adjustment_temp')->row();
    ...
    function min_stock_temp($pid,$aqty,$so)
    {
        $this->ci->db->where('product_id', $pid);
        $this->ci->db->where('stock_adjustment', $so);
        $val = $this->ci->db->get('stock_adjustment_temp')->row();
        $qty = intval($val->qty - $aqty);

        $res = array('qty' => $qty);
        $this->ci->db->where('id', $val->id);
        $this->ci->db->update('stock_adjustment_temp', $res);
    ...
    {
        $this->ci->db->where('stock_adjustment', $no);
        $result = $this->ci->db->get('stock_adjustment_temp')->result();
        
        foreach ($result as $res)
        {
            $this->do_rollback($res->id, $res->qty, $res->stock_adjustment);
        }
        
        $this->ci->db->where('stock_adjustment', $no);
        $this->ci->db->delete('stock_adjustment_temp');
    }
    ...
    function get_amount_stock($no)
    {
      ...
      $this->ci->db->where('stock_adjustment', $no);
      $val = $this->ci->db->get('stock_adjustment_temp')->row_array();
    ...
    {       
       $trans = array('id' => $id, 'product_id' => $pid, 'dates' => $date, 'transdates' => $transdate, 
                      'qty' => $qty, 'amount' => $amount, 'stock_adjustment' => $so);
       
       $this->ci->db->insert('stock_adjustment_temp', $trans);
    ...
    function get_beginning_balance($product,$start,$end)
    {
       ...
       return $this->ci->db->get('stock_adjustment_temp')->row_array();
    }
